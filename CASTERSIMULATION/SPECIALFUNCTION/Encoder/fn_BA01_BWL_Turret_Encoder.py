...
__all__ = ['Fn_BA01_BWL_Turret_Encoder']
...
class Fn_BA01_BWL_Turret_Encoder():

    def __init__(self, filename):

        self.devicename = 'BA01_BWL_Turret_Encoder'
    ...
    def setup(self):
        try:
            ...
            self.areatag = str(621.1)
            self.speedFB = str('db1412.dbd176')
            self.encoderoutputtag = str(3610)
            self.mtrnominalSpd = int(1770)
            self.plcscancycle = float(0.1)
            self.driveEngg = int(16384)
            self.kval = int(1)
            self.encodertype = str('incremental')
            self.resolution = int(4096)

        except Exception as e:
            level = logging.ERROR
            messege = "BA01_BWL_Turret" + self.devicename + " Error messege(setup)" + str(e.args)
