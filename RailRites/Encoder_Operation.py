...
class Encoder_Operation:
    def __init__(self,root,gen):
        self.root = root
        self.gen = gen
        self.df = pd.read_excel(r'C:\OPCUA\Working_VF1_5.xls', sheet_name='Encoder')
    ...
    def update(self):
            for i in range(0, len(self.df.index)):
                ...
                if self.tagname_entered.get() == self.df.iloc[i, 0]:
                    ...
                    self.df.iloc[i, 6] = self.fastcount.get()
    ...
    def collectwritetaglist(self):
        ...
        while n < len(self.df.index):
            self.listofdevice.append(self.df.iloc[n, 0])
            self.listofencoderoutputtags.append(self.df.iloc[n, 7])
            n = n + 1
