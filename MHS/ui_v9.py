...
class FormUi:
    ...
    def initilization(self):
        ...
        try:
            ...
            self.vibrofeederobject = allvibrofeedersprocessing.vibrofeederprocess(self.alldevices,self.import_file_path)
    ...
    def callallvibrofeeder(self,com,devices):

        while not self.DEAD:
            self.vibrofeederobject.process()
    ...
    def vfstart(self):
        self.DEAD = False
        self.vftread = threading.Thread(target=self.callallvibrofeeder, args=(self.comm_object, self.alldevices))
        self.listofthread.append(self.vftread)
        self.vftread.start()
        self.vftreadstartbutton.configure(text="vibrofeederstarted")
    ...
    def startprocess(self):
        ...
        self.vftreadstartbutton = ttk.Button(self.win, text='vibrofeederstart', command=self.vfstart)
        self.vftreadstartbutton.grid(column=0, row=0)

        self.motor2dstartbutton = ttk.Button(self.win, text='motor2dstart', command=self.motor2dstart)
        self.motor2dstartbutton.grid(column=1, row=0)


        self.rampstartbutton = ttk.Button(self.win, text='rampstart', command=self.rampstart)
        self.rampstartbutton.grid(column=0, row=1)
        self.sov1startbutton = ttk.Button(self.win, text='sov1start', command=self.sov1start)
        self.sov1startbutton.grid(column=1, row=1)
    ...
    def stopprocess(self):
        ...
        self.DEAD = True
        self.vftreadstartbutton.configure(text = 'vibrofeederstart')
...
class App:
    def __init__(self, root):
        self.root = root
        root.title('SMS ESSER MHS PLC SIMULATION')
    ...
    def quit(self, *args):
        self.clock.stop()
        os.system("taskkill /f /im  MH_LF.exe")
        self.root.destroy()
