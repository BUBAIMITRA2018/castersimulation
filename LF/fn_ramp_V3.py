...
class Fn_Ramp(Eventmanager):
    ...
    def increaserampprocess(self):
        ...
        try:
            ...
            self.currentpv = ((self.highlimit - self.lowerlimit) * (self.currentrawvalue / 27648))
            ...
            if self.comparevalue > self.currentpv:
                t1 = threading.Thread(target=self.increasefunction,
                                      args=(self.currentpv, self.comparevalue, self.positiverate))
                t1.start()
                # t1.join()
    ...
    def processvalue(self):
        ...
        return float((self.highlimit - self.lowerlimit) * (currentrawvalue / 10000))
    ...
    def increasefunction(self,currentpv, targetsp, rate):
        client = Communication()
        sta_con_plc = client.opc_client_connect(self.filename)
        writegeneral = WriteGeneral(sta_con_plc)
        readgeneral = ReadGeneral(sta_con_plc)
