...
def driving(a, f,
            t):  # The driving force.  This is a function of time. f is the frequency of the driving force and a is its maximum value
    driving_force = a * sin(2 * pi * f * t)
    return driving_force


def cycle(my_list,
          new):  # Takes a three item list, moves the elements down one place and adds a new element into position [2]
    my_list[0] = my_list[1]
    my_list[1] = my_list[2]
    my_list[2] = new
    return my_list
...
def simulate_oscillations(driving_force_amplitude, driving_frequency, drag_coefficient, k, m, dt,
                          option):  # This does all the work.

    x = 0  # Initial dispalcement
    t, v = 0, 0  # Initial time and speed

    x_list = [0, 0,
              0]  # This list keeps track of the three previous x values. If the number in the middle is the largest then
    ...
    while flag == 0:  # This loop keeps going until we are satisfied that the amplitude has reached a stable, maximum value
        ...
        r = f(k, x) + d(drag_coefficient, v) + driving(driving_force_amplitude, driving_frequency,
                                                       t)  # Calculates the resultant force on the mass
        ...
        if (x_list[1]) > (x_list[0]) and (x_list[1]) > (
        x_list[2]):  # Checks to see if x_list[1] is larger than x_list[0] and x_list[2]
        ...
        if l > 3:  # Wait until we have 3 amplitues to compare
            if fabs(amplitude_list[l - 1] - amplitude_list[l - 2]) < 1e-5 and fabs(
                    amplitude_list[l - 2] - amplitude_list[
                        l - 3]) < 1e-5:  # If the amplitude is changing by less than 0.0001 each cycle then it is constant
...
def run_the_experiment(a, b, c):
    ...
    while run <= c:
        results.append(simulate_oscillations(3, a, 0.4, 20, 0.5, 0.001,
                                             'no_graph'))  # (driving_force_amplitude, driving_frequency, drag_coefficient, k, m, dt, option)
        f_list.append(a)
        a = a + step_size
        run = run + 1
...
run_the_experiment(0.1, 1.5,
                   100)  # run_the_experiment(begin frequecy range, end of frequency range, number of points between a and b)
