...
class Fn_Sov1S(Eventmanager):
    ...
    def initilizedigitalinput(self):
        ...
        if len(self.openFBtag) > 3:
            self.gen.writegeneral.writesymbolvalue(self.openFBtag,'digital',0)
            level1 = logging.INFO
    ...
    def sov1sprocess(self):
        try:
            ...
            self.cmdtagvalue = self.gen.readgeneral.readsymbolvalue(self.cmdtag,"digital")
            if self.cmdtagvalue:
                self.gen.writegeneral.writesymbolvalue(self.closeFBtag, 'digital', 0)
                self.gen.writegeneral.writesymbolvalue(self.openFBtag,'digital',1)
