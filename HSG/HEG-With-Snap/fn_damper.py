...
class Fn_Damper(Eventmanager):
    ...
    def __init__(self,com,df,idxNo,filename):
        ...
        super().__init__(lambda: self.damperprocess())
    ...
    def setup(self):
        try:
            ...
            level = logging.ERROR
            messege = "FN_Damper" + self.devicename + " Error messege(setup)" + str(e.args)
    ...
    def initilizedigitalinput(self):
        try:
            ...
            if len(self.faultfb)>3:
                writegeneral.writesymbolvalue(self.faultfb, 0, 'S7WLBit')
                level = logging.INFO
                messege = self.devicename + ":" + self.faultfb + " is trigger by 0"
                logger.log(level, messege)

            if len(self.remotefb)>3:
                writegeneral.writesymbolvalue(self.remotefb, 1, 'S7WLBit')
                level = logging.INFO
                messege = self.devicename + ":" + self.remotefb + " is trigger by 1"
                logger.log(level, messege)

            if len(self.tempswfb) > 3:
                writegeneral.writesymbolvalue(self.tempswfb, 1, 'S7WLBit')
                level = logging.INFO
                messege = self.devicename + ":" + self.tempswfb + " is trigger by 1"
                logger.log(level, messege)
    ...
    def damperprocess(self):
        ...
        try:
            ...
            print(self.opncmdvalue)
            print(self.clscmdvalue)
            print(self.openFBtag)
            print(self.closeFBtag)
            print(self.openLS)
            print(self.closeLS)
            ...
            if self.opncmdvalue == True and self.clscmdvalue == False:
                ...
                if len(self.postion)>3:
                    writegeneral.writesymbolvalue(self.postion, 6912, 'S7WLWord')
                    sleep(1)
                    writegeneral.writesymbolvalue(self.postion, 13828, 'S7WLWord')
                    sleep(1)
                    writegeneral.writesymbolvalue(self.postion, 20736, 'S7WLWord')
                    sleep(1)
                    writegeneral.writesymbolvalue(self.postion, 27648, 'S7WLWord')
                else:
                    sleep(5)
            ...
            if self.opncmdvalue == False and self.clscmdvalue == True:
                ...
                if len(self.postion)>3:
                    ...
                    writegeneral.writesymbolvalue(self.postion, 6912, 'S7WLWord')
                    sleep(1)
                    writegeneral.writesymbolvalue(self.postion, 0, 'S7WLWord')
                else:
                    sleep(5)
