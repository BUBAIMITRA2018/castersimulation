...
class AreaObserver:
    ...
    def notify(self,  *args, **kwargs):
        try:
            ...
            log_exception(e)
            level = logging.ERROR
            messege = "damperprocess:" + " Exception rasied(AreaObserver): " + str(e.args) + str(e)
...
class damperprocess:
    ...
    def process(self):
        try:
            ...
            log_exception(e)
            level = logging.ERROR
            messege =  "damperprocess:" + " Exception rasied(process): " + str(e.args) + str(e)
            logger.log(level, messege)


def readkeyandvalues(alldevice):

         damperdictionary = alldevice.alldamper.dictionary

         areas = list(damperdictionary.keys())
         n = 0
         while n < len(areas):
             area = areas[n]
             devices = damperdictionary[area]
             yield area,devices
             n = n + 1
