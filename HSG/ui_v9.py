...
class FormUi:
    ...
    def snap(self):
        ...
        while p < row:
            ...
            dict.update(keyvalue)
            p = p+1
        with open(self.directory+'\snap'+str(datetime.datetime.now()).replace(" ","_").replace(".","_").replace(":","_")+'.txt', 'w') as json_file:
    ...
    def initilization(self):
        ...
        try:
            ...
            self.damperobject = alldamperprocessing.damperprocess(self.alldevices, self.import_file_path)
    ...
    def calldampers(self,com,devices):
        while not self.DEAD:
            self.damperobject.process()
            # time.sleep(2)
    ...
    def damperstart(self):
        self.DEAD = False
        self.dampertread = threading.Thread(target=self.calldampers,args=(self.comm_object, self.alldevices))
        self.dampertread.start()
        self.damperstartbutton.configure(text="damperstarted")
    ...
    def startprocess(self):
        ...
        # self.analogstartbutton.grid(column=1, row=0)


        # self.controlvalvestartbutton = ttk.Button(self.win, text='controlvalstart', command=self.controlvalvestart)
        # self.controlvalvestartbutton.grid(column=1, row=1)

        self.digitalstartbutton = ttk.Button(self.win, text='digitalvalstart', command=self.digitalstart)
        self.digitalstartbutton.grid(column=1, row=0)

        self.damperstartbutton = ttk.Button(self.win, text='damperstart', command=self.damperstart)
        self.damperstartbutton.grid(column=1, row=1)
    ...
    def stopprocess(self):
        ...
        self.damperstartbutton.configure(text='damperstart')
...
class App:
    ...
    def quit(self, *args):
        self.clock.stop()
        os.system("taskkill /f /im  HEG.exe")
        self.root.destroy()
